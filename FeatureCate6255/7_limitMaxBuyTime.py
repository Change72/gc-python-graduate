...
for brand in brandList:
    ...
    for row in userInfo:
        if row[2] == "True" and row[7] != "0" and row[1] == brand:
            row[25] = math.floor(float(row[25]))
            if i == 0:
                i = i + 1
                brandMinViewTime = row[25]
                brandMaxViewTime = row[25]
                continue

            if 0 <= row[25] < brandMinViewTime:
                brandMinViewTime = row[25]
                continue

            if row[25] > brandMaxViewTime:
                brandMaxViewTime = row[25]
                continue

    if brandMaxViewTime > 7200:
        brandMaxViewTime = 7200
        brandMinViewTime = 0
    ...
    if brandMaxViewTime > 0:
        ...
        for row in userInfo:
            if row[2] == "True" and row[7] != "0" and row[1] == brand:
                row[25] = math.floor(float(row[25]))
                targetColumn = int(math.floor((row[25] - brandMinViewTime) / eachGap))
        ...
        ax.set_title("User with Brand " + str(brand) + " Pv to Buy Time Interval Before First Purchase")
        ax.set_xlabel("Pv to Buy Time Interval")
        ax.set_ylabel("User Number")

        plt.savefig("../FeatureCate" + str(cate) + "/limitedFirstPvtoBuyTimeDistribution/" + str(brand) + "PvtoBuyTime.png")