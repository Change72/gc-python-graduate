...
for row in userInfo:
    if i == 0:
        i = i + 1
        label = [row[1], row[25], row[26], row[27], row[28], row[29], row[30], row[31], row[32], row[13], row[33],
...
X_resampled, y_resampled = SMOTEENN().fit_sample(temparray, y)
# c = list(zip(X_resampled, y_resampled))
# random.shuffle(c)
# X_resampled[:], y_resampled[:] = zip(*c)
# temparray = X_resampled
# y = y_resampled
# m = len(temparray)
for i in range(n):
    for j in range(i + 1, n):
        firstFea = X_resampled[:, i]
        secondFea = X_resampled[:, j]
        fig = plt.figure(figsize=(9.6, 5.4))
        ax = fig.add_subplot(111)
        ax.set_title("SMOTE of Feature " + str(i) + " and " + str(j) + " Scatter Plot")
        plt.xlabel(label[i])
        plt.ylabel(label[j])
        trueIdx = myfind(1, y_resampled)
        ax.scatter(firstFea[trueIdx], secondFea[trueIdx], c='r', marker='o')
        falseIdx = myfind(-1, y_resampled)
...
# model2.fit_transform(temparray, y_resampled)
