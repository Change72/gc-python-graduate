...
#     if usestatus:
#         return float((1.0 / (1 + np.exp(- float(x))) - 0.5) * 2)
#     else:
#         return float(x)
...
# 不画折线图的原因是有大量的0
plt.bar(list(range(32)), featureScore)
...
# fig = plt.figure(figsize=(9.6, 5.4))
# # 不画折线图的原因是有大量的0
# plt.bar(list(range(0, maxFeature)), pca.explained_variance_ratio_)
#
# ax = fig.add_subplot(1, 1, 1)
#
# ax.xaxis.grid(True, which='major')  # x坐标轴的网格使用主刻度
