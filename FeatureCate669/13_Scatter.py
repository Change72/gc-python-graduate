import pandas as pd
import matplotlib.pyplot as plt
import csv
import numpy as np


def myfind(x, y):
    idx = []
    for row in list(range(len(y))):
        if y[row] == x:
            idx.append(row)
    return idx


cate = 669
userFile = "second_fourth_final_tryKMeansOfCate" + str(cate) + ".csv"
userInfo = csv.reader(open(userFile, 'r'))

# 初始化最大值列表和最小值列表
i = 0
feature = []
y = []
trueNum = 0
fraudNum = 0
label = []
for row in userInfo:
    if i == 0:
        i = i + 1
        label = row
        continue
    else:
        # if row[37] == "T":
        #     row[37] = 1
        #     trueNum = trueNum + 1
        # else:
        #     row[37] = -1
        #     fraudNum = fraudNum + 1

        tag = [row[1], row[4], row[5], row[6], row[7], row[8], row[9], row[10], row[11], row[12], row[13], row[14],
               row[15], row[16], row[17], row[18], row[19], row[20], row[21], row[22], row[23], row[24], row[25],
               row[26], row[27], row[28], row[29], row[30], row[31], row[32], row[33], row[34]]
        feature.append(tag)
        y.append(row[37])

temparray = np.array(feature, dtype=float)
m, n = temparray.shape
# 归一化每一个特征

firstFea = temparray[:, 28]
secondFea = temparray[:, 29]

fig = plt.figure()
ax = fig.add_subplot(111)
ax.set_title("Scatter Plot")
plt.xlabel(label[31])
plt.ylabel(label[32])
trueIdx = myfind("T", y)
ax.scatter(firstFea[trueIdx], secondFea[trueIdx], c='r', marker='o')
falseIdx = myfind("F", y)
ax.scatter(firstFea[falseIdx], secondFea[falseIdx], c='g', marker='*')
plt.show()


# for j in range(n):
#     features = temparray[:, j]
#     meanVal = np.mean(features, axis=0)
#     std = np.std(features, axis=0)
#     if std != 0:
#         temparray[:, j] = (features - meanVal) / std
#     else:
#         temparray[:, j] = 0











# i = 0
# featureLength = 37
# featureList = list(range(0, featureLength))
# for k in featureList:
#     featureList[k] = []
# print(featureList)
# unusedFeatureList = [0, 2, 3, 35, 36]
# # 特征转置
# for row in userInfo:
#     if i == 0:
#         i = i + 1
#         continue
#     else:
#         for k in featureList:
#             if k != featureLength - 1:
#                 featureList[k].append(float(row[k]))
#             else:
#                 featureList[k].append(row[k])
#

